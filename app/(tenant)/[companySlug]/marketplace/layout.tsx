...
export default function MarketplaceLayout({
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        if (search.trim()) {
            router.push(`/${companySlug}/marketplace?search=${encodeURIComponent(search.trim())}`);
        }
    };
        ...
        <div className="min-h-screen bg-background text-foreground flex flex-col font-sans transition-colors duration-300">
            ...
            <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-2xl border-b border-border">
                <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between gap-6">
                    {/* Logo */}
                    <Link href={`/${companySlug}/marketplace`} className="flex items-center shrink-0">
                    ...
                    <div className="hidden md:flex items-center space-x-4">
                        ...
                        <Link href={`/${companySlug}/profile/settings`}>
                        ...
                        <Link href={`/${companySlug}/marketplace/post-ad`}>
                ...
                <div className="bg-secondary/30 border-t border-border overflow-x-auto scrollbar-hide">
                    <div className="max-w-7xl mx-auto px-4 flex items-center py-3 space-x-8 whitespace-nowrap text-sm font-medium text-muted-foreground">
                        {['Lab Equipment', 'Medical Supplies', 'Reagents', 'Diagnostics', 'Surgical', 'Accessories', 'Pharma'].map((cat) => (
                            <Link
                                key={cat}
                                href={`/${companySlug}/marketplace?category=${cat.toLowerCase()}`}
                    ...
                    <div className="md:hidden absolute top-20 left-0 w-full bg-background border-b border-border animate-in slide-in-from-top duration-300 z-40 overflow-hidden shadow-2xl">
                        <div className="p-6 space-y-6">
                            <div className="space-y-4">
                                <Link href={`/${companySlug}/cart`} onClick={() => setIsMenuOpen(false)} className="flex items-center justify-between p-4 bg-secondary/50 rounded-2xl group active:scale-[0.98] transition-all">
                                ...
                                <Link href={`/${companySlug}/profile/settings`} onClick={() => setIsMenuOpen(false)} className="flex items-center justify-between p-4 bg-secondary/50 rounded-2xl group active:scale-[0.98] transition-all">
                                    <div className="flex items-center space-x-4">
                                ...
                                <Link href={`/${companySlug}/marketplace/post-ad`} onClick={() => setIsMenuOpen(false)} className="flex items-center justify-between p-4 bg-primary text-primary-foreground rounded-2xl group active:scale-[0.98] transition-all shadow-lg shadow-primary/20">
            ...
            <footer className="bg-secondary/50 border-t border-border py-12">
                <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8">
                    <div className="col-span-1 md:col-span-2">
                        <Link href={`/${companySlug}/marketplace`} className="flex items-center mb-6">
                            <img src={logoPath} alt="Logo" className="h-8 w-8 rounded-lg object-contain shadow-lg shadow-primary/20" />
                        </Link>
                        <p className="text-muted-foreground max-w-sm mb-6 font-medium">The ultimate destination for healthcare solutions. Verified sellers, secure payments, and worldwide shipping.</p>
                    </div>
                    <div>
                        <h4 className="font-bold text-foreground mb-4">Quick Links</h4>
                        <ul className="space-y-2 text-muted-foreground text-sm font-medium">
                            <li><Link href={`/${companySlug}/marketplace`} className="hover:text-primary transition-colors">Browse Ads</Link></li>
                            <li><Link href={`/${companySlug}/marketplace/post-ad`} className="hover:text-primary transition-colors">Sell Something</Link></li>
                            <li><Link href={`/${companySlug}/profile/my-ads`} className="hover:text-primary transition-colors">My Ads</Link></li>
                        </ul>
                    </div>
                    <div>
                ...
                <div className="max-w-7xl mx-auto px-4 mt-12 pt-8 border-t border-border text-center text-muted-foreground text-xs font-semibold">
                    &copy; {new Date().getFullYear()} Unique Healthcare Solutions. All rights reserved.
                </div>
            </footer>
        </div>
    );
