...
export default function AlphasoftBanner({ variant = 'default' }: AlphasoftBannerProps) {
    ...
    return (
        <div className={`relative group overflow-hidden rounded-2xl bg-linear-to-r from-indigo-600 via-purple-600 to-pink-600 p-px shadow-2xl transition-all duration-500 hover:shadow-primary/20 ${isCompact ? 'w-full' : ''}`}>
            <div className={`relative flex items-center justify-between gap-2 rounded-[15px] bg-background/90 backdrop-blur-xl transition-all duration-500 group-hover:bg-background/80 ${isCompact ? 'px-3 py-2 flex-col items-stretch' : 'px-4 py-3'}`}>
                ...
                <div className="absolute -left-4 -top-4 h-24 w-24 bg-primary/10 blur-3xl transition-opacity group-hover:opacity-100 opacity-0" />
                <div className="absolute -right-4 -bottom-4 h-24 w-24 bg-purple-500/10 blur-3xl transition-opacity group-hover:opacity-100 opacity-0" />
            </div>
        </div>
    );
}
